...
class CreateNewDataBase:
  ...
  #   En étant titulaire d’un diplôme de formation supérieur ou égal au brevet de maitrise, après avoir exercé son activité pendant 2 ans et après validation par la commission régionale de qualification.
  #   Être immatriculé au répertoire des métiers depuis plus de 10 ans et justifier auprès de la commission régionale de qualification d’un savoir-faire mis au service de la promotion de l’artisanat (participation à des travaux de recherche, à des évènements…) ou de la participation à des actions de formation (jury d’examen, formation d’apprentis…)"		x
  ...
  def fillupUserProfile(self, table):
    UserProfile.objects.create(userNameInternal=User.objects.get(username="jlw"), Company=Company.objects.get(name="test"), firstName="Jeanluc", lastName="Walter", role=Role.objects.get(id=1), cellPhone="06 34 09 06 95", token=None)
